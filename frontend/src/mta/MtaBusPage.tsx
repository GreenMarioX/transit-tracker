...
import BusMap from '@/components/ui/bus-map';
...
const MtaBusPage = () => {
  ...
  const [routesByBoro, setRoutesByBoro] = useState([])
  ...
  const fetchBusStopsData = async (route) => {
    try {
  ...
  const findNameById = (stopId: string | number) => {

    const idToFind = "MTA_" + stopId;
    let item = null

    if (busStops && busStops.zeroDirStopsData) {
      // Search in oneDirStopsData
      if (stopId && Array.isArray(busStops.zeroDirStopsData)) {
          item = busStops.zeroDirStopsData.find((obj) => {
    ...
    if (busStops && busStops.oneDirStopsData) {
      if (Array.isArray(busStops.oneDirStopsData)) {
          item = busStops.oneDirStopsData.find((obj) => {
            return obj.id == idToFind
          });
      }
  ...
  function extractSixDigitNumber(inputString) {
  ...
  return (
    ...
    <div className="container mx-auto py-10 flex gap-x-8 overflow-auto">
                  ...
                  ? busStops.zeroDirStopsData.map((stop) => (
                      <React.Fragment key={stop.id}>
                        <div
                          ...
                          onClick={() => setStopId(extractSixDigitNumber(stop.id))}
                        >
                          {stop.name}
                        </div>
      ...
      <div className="w-10/12 overflow-scroll">
        <div>
            <h2 className="text-2xl font-bold tracking-tight">Stop: {findNameById(stopId)}!</h2>
